...
switch(@$_GET['page']){
    ...
    case 'save';
        if($_SERVER['REQUEST_METHOD']=="POST"){
            ...
            if(empty($_POST['ket'])){
                $err['ket']="Wajib ISI Keterangan Barang";
            }
            //validasi file input
            ...
            if(!isset($err)){
                ...
                if(isset($_POST['photo'])){
                    ...
                    id_pemasok='$_POST[id_pemasok]',id_admin=$id_admin, photo='$_POST[photo]', ket='$_POST[ket]' where md5(id_barang)='$_POST[id_barang]'";
                }else{
                $sql="UPDATE barang set nama_barang='$_POST[nama_barang]',harga='$_POST[harga]', id_jenis='$_POST[id_jenis]',
                id_pemasok='$_POST[id_pemasok]',id_admin=$id_admin, ket='$_POST[ket]' where md5(id_barang)='$_POST[id_barang]'";
                }    
                }else{
                //input
                if(isset($_POST['photo'])){
                    $sql = "INSERT INTO barang (nama_barang, harga, id_jenis, id_pemasok, id_admin, photo, ket)
                VALUES ('$_POST[nama_barang]', '$_POST[harga]','$_POST[id_jenis]','$_POST[id_pemasok]',
                $id_admin, '$_POST[photo]', '$_POST[ket]')";
                }else{
                $sql = "INSERT INTO barang (nama_barang, harga, id_jenis, id_pemasok, id_admin, ket)
                VALUES ('$_POST[nama_barang]', '$_POST[harga]','$_POST[id_jenis]','$_POST[id_pemasok]',
                $id_admin, '$_POST[ket]')";
                }
            }
    ...
    if (ISSET($_POST['submit'])){
        $cari = $_POST['nt'];
        $sql="SELECT* FROM `barang` INNER JOIN jenis_barang ON barang.`id_jenis`=jenis_barang.`id_jenis`
        INNER JOIN pemasok ON barang.`id_pemasok`=pemasok.`id_pemasok` where nama_barang LIKE '%$cari%';";
        $penyedia=$conn->query($sql);
        $conn->close();
